...
### Last updated: July 23, 2026 (afternoon — flag #82 raised: frontmatter-audit `.venv` scope gap)
...
| 82 | `frontmatter_audit.py`'s `EXCLUDED` set (line 28) does not exclude `.venv\` or other Python-environment folders. Today's MCP Bootcamp Day 5 `.venv` creation (`02-LIBRARY\.PROJECTS\MCP_Bootcamp\.venv\`) pulled in 8 pip-installed third-party `LICENSE.md`/`NOTICE.md` files from site-packages, all reported as new `missing_frontmatter` findings and tripping `root_health.py` to BLOCKER even though nothing Chris authored is actually missing metadata — same false-positive class as closed flag #81 (script scope, not real content debt). Will recur for any future project `.venv`. | July 23 | LOW | `00-BRAIN\scripts\frontmatter_audit.py` line 28 — add `.venv`, `venv`, `node_modules` (and similar dependency-manager folders) to `EXCLUDED` | OPEN — confirmed via `root_health.py --verbose`, all 8 new findings traced to `MCP_Bootcamp\.venv\Lib\site-packages\`; the one other new/unresolved finding (`Clippings\Microsoft Privacy Statement.md`, missing type/timeline) is the same pre-existing genuine gap already tracked, not new |
---
