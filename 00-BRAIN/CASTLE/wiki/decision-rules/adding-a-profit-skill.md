---
type: decision-rule
status: active
tags: [decision-rule, scope-control, reference]
---

# Decision Rule: Adding a Profit Skill

**Trigger**: Any new "this could be highly profitable" idea — a skill, tool, market,
side hustle, or digital-asset concept — from Chris, an AI session, or any source.
**Owner**: Chris decides; AI applies the gate and flags, never silently adopts.

## The Rule

A new profit skill enters the [[skill-map]] only if ALL five pass:

1. **No-orphan test** (NORTH_STAR.md): it directly serves school, a defined service
   on the roadmap, or a currently active project. "Might be useful someday" fails.
2. **Source test**: at least one Tier 1–2 source (registered in [[source-map]])
   supports the claim that this skill produces income for someone in Chris's
   position. Tier 4 hype (Reddit threads, YouTube income claims, X posts) can
   trigger the question but can never pass this test by itself.
3. **Phase test**: it names the phase it belongs to. If that phase is more than
   two quarters away → `parked-advanced/`, not the skill map.
4. **Displacement test**: name what it displaces. Time is fixed (full-time school,
   family of nine). If nothing can be displaced, it waits.
5. **Proof test**: a concrete proof project can be stated in one sentence. If the
   proof can't be named, the skill isn't ready to be learned.

**Timing override**: during danger weeks and heavy semesters, the answer is
automatically "parked" — evaluate after.

## Why This Rule Exists

NORTH_STAR.md names the #1 risk: "Idea generation aptitude is highest tier. The same
instinct that finds business problems also generates more projects than there is
time to complete." This rule is that defense, made mechanical. The existing plan
already contains the profit engine (audit → build → retainer → productize); most
new ideas are worse uses of the same hours.

## Worked Example

*Idea (July 2026): "Day trading — there's a whole shelf of books in 02-LIBRARY."*
1. No-orphan: serves no track, no service, no active project — **fail**.
2. Source: books exist (Tier 2) but support a different career, not this roadmap — fail.
Verdict: **parked** (which is exactly where those books already are). Two failures,
no page, no time spent. The rule took 60 seconds.

*Counter-example that PASSES: "Report-generation pipeline (Markdown → PDF)."*
Serves the audit service (Phase 3–5), the BUSINESS/PYTHON wikis' Tier-1 sources cover it, phase is near,
displaces nothing (it's inside POL scope), proof = POL weekly report as PDF. → skill map.

## When to Break It

None. If an idea feels like the exception, that feeling is the risk firing.
Log it in the parking lot and re-run the gate at the next monthly review.
