...
|---|---|---|---|---|
| 2026-08-22 | College-oriented AI study OS pattern: local course corpus, citation-grounded retrieval, adaptive practice, and LMS-fed planning | **HOLD** full-platform adoption; one-course ClassCorpus evaluation candidate only | `.ROOT` already owns official-source truth, semester-ahead sequencing, learner proof, grades/misses, and course-policy boundaries. The reviewed full stacks duplicate authority or add large/cloud/institutional dependencies; D2L acquisition and exact mixed-file citations are the only plausible gaps, and neither is yet measured in live semester use | After Aug 24 D2L reconciliation, unlock only if Week 1–2 produces two >2-minute retrievals, one stale-index error, or one study-block delay from mixed-file search; then run the seven-check supervised pilot in the AIAS evidence page |
