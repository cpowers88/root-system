---
type: guide
timeline: reference
status: live
tags: [physics, school]
---

# PHYSICS Wiki

Sequential calculus-based physics preparation and durable engineering
competence.

## Route

- Human workflow: `HOW_TO_USE.md`
- Current learner truth: `wiki/current-position.md`
- Active navigation: `wiki/index.md`
- Durable stage sequence: `wiki/learning-path.md`
- Semester pacing: `wiki/pacing-trigger-map.md`
- Machine contract: `OPERATIONS.md`
- AI loader: `CLAUDE.md`

Current frontier: Stage 4, Motion in Two Dimensions. Confirm the exact next rep
in `wiki/current-position.md`; do not infer progress from generated packets.

`raw/` is immutable. Official exact-section course material overrides this
wiki.
