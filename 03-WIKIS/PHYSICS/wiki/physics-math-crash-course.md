...
Example: solve centripetal acceleration `a_c = v^2/r` for `v`.

$$a_cr=v^2 \quad\Rightarrow\quad v=\sqrt{a_cr}$$
...
$$v_{avg}=\frac{15-3}{6-2}=3\ \mathrm{m/s}$$
...
For launch speed `v_0` at angle `theta_0`:

$$v_{0x}=v_0\cos\theta_0,\qquad
v_{0y}=v_0\sin\theta_0$$

Horizontal lane:

$$a_x=0,\qquad v_x=v_{0x},\qquad x=x_0+v_{0x}t$$

Vertical lane:

$$a_y=-g,\qquad v_y=v_{0y}-gt,
\qquad y=y_0+v_{0y}t-\frac12gt^2$$
...
$$v_{0x}=20\cos30^\circ=17.3\ \mathrm{m/s}$$
$$v_{0y}=20\sin30^\circ=10.0\ \mathrm{m/s}$$

At the peak, vertical velocity is zero:

$$0=10.0-9.80t \Rightarrow t_{peak}=1.02\ \mathrm{s}$$

Height:

$$y_{max}=10.0(1.02)-4.90(1.02)^2=5.10\ \mathrm{m}$$

Because launch and landing heights match, the downward half mirrors the upward
half in time:

$$t_{flight}=2.04\ \mathrm{s}$$

Horizontal range:

$$R=v_{0x}t=17.3(2.04)=35.3\ \mathrm{m}$$
...
torque:                   tau = dL/dt
force from potential:     F_x = -dU/dx
```
...
The initial condition `v(0)=v_0` makes `C=v_0`, so

$$v(t)=v_0+at$$

Integrating again and applying `x(0)=x_0` gives
...
| `F=ma` | `tau=I alpha` |
| `K=1/2 mv^2` | `K_rot=1/2 I omega^2` |
...
- `a_c=v^2/r` goes to zero when `v -> 0`.
...
The pendulum's familiar small-angle period depends on `sin theta approximately
theta`. The approximation fails at larger angles.
...
|---|---|
| `Algebra_Cheat_Sheet.pdf` | exponents, radicals, factoring, quadratics, functions, common algebra errors |
...
| `Common_Derivatives_Integrals.pdf` | derivative and antiderivative lookup |
...
4. **Trig:** `F_x=164 N`, `F_y=115 N`; `sqrt(F_x^2+F_y^2)=200 N` after rounding.
5. **Vectors:** `R_x=33.0 N`, `R_y=62.7 N`, `R=70.9 N`, approximately
