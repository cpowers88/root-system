...
| 3 | Sat Aug 1 | Cold rebuild: derive all three 1D kinematics equations from a = const, no formula sheet. Flag v² = v₀² + 2aΔx explicitly — it's algebraic elimination of t between the other two, not a third integration (~~planned, did not run~~ — **RAN AND PASSED 2026-08-17**, 16 days late. Both integrations rebuilt cold with constants named physically; derive-vs-remember probe clean (`C₁t → v₀t` was a deliberate substitution, not recall); transfer problem passed. **Open habit: reasonableness check dropped a third time** — taught once as "same number by a different road," watch on row 4. See `log.md` 2026-08-17) |
