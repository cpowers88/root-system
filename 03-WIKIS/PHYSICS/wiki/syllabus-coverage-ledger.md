---
type: map
timeline: reference
status: active
reference_priority: core
tags: [physics, school, governance]
---

# Syllabus Coverage Ledger

## Purpose

Account for every page of `syllabus.pdf` (the original provisional spine source)
without mixing course operations into physics lesson packets. Academic content
routes to the Stage 1-18 path; policies and resources route to the
course-governance layer. This ledger was completed by visual inspection of all 19
pages on 2026-07-15.

**Source note (2026-07-23):** `syllabus.pdf` was superseded and archived on
2026-07-21 — it no longer lives at `raw/syllabus/`; the current copy is
`99-ARCHIVE\02-LIBRARY\00-SCHOOL\SYLLABI_REPLACED_2026-07-21\02-Physics I\syllabus.pdf`.
The page-by-page disposition and data-quality findings below are preserved as
historical record. The two real Fall 2026 section syllabi in `raw/syllabus/`
(Section 51 and Section 55 — see [[source-map]] and Cross-Section Verification
below) are now the current best-available sources for anything past Chapter 5.

## Authority Rule

1. Use the syllabus for course identity, learning outcomes, grading categories,
   academic-integrity boundaries, and the named physics scope.
2. Use the live D2L/WebAssign/Owl Express course for dates, deadlines, assignment
   availability, exam/drop mechanics, and any item flagged below as conflicting.
3. Use Serway & Jewett as the content spine and [[learning-path]] as the staged route.

## Academic Content Routed to Stages

| Syllabus statement | Stage route | Coverage proof |
|---|---|---|
| measurements and course introduction | Stage 1 | [[stages/stage-1-physics-and-measurement]] |
| kinematics, falling objects, graphical/pictorial/mathematical representations | Stages 2-4 | [[stages/stage-2-motion-in-one-dimension]], [[stages/stage-3-vectors]], [[stages/stage-4-motion-in-two-dimensions]] |
| Newton's laws and dynamics | Stages 5-6 | [[stages/stage-5-laws-of-motion]], [[stages/stage-6-circular-motion]] |
| conservation of energy | Stages 7-8 | [[stages/stage-7-energy-of-a-system]], [[stages/stage-8-conservation-of-energy]] |
| conservation of linear and angular momentum | Stages 9-11 | [[stages/stage-9-linear-momentum]], [[stages/stage-10-rotation]], [[stages/stage-11-angular-momentum]] |
| broader classical mechanics and equilibrium | Stage 12 | [[stages/stage-12-static-equilibrium]] |
| universal gravitation, falling/orbital objects | Stage 13 | [[stages/stage-13-universal-gravitation]] |
| fluid mechanics confirmed in the semester route | Stage 14 | [[stages/stage-14-fluid-mechanics]] |
| simple harmonic motion and oscillations | Stage 15 | [[stages/stage-15-oscillatory-motion]] |
| waves, superposition, and standing waves | Stages 16-17 | [[stages/stage-16-wave-motion]], [[stages/stage-17-superposition]] |
| basic special relativity | Stage 18 | [[stages/stage-18-relativity]] |
| algebra, trigonometry, and calculus used in physical problem solving | just-in-time across all stages | [[math-readiness-path]], [[calculus-map]], [[appendix/math-algebra]], [[appendix/math-geometry-trig]], [[appendix/math-calculus]] |

## Course Outcomes Applied Across Every Stage

Every activated stage must require Chris to:

1. explain the physical situation in plain language;
2. construct an appropriate sketch, graph, physical model, or free-body diagram;
3. identify assumptions and choose a symbolic relationship before substituting;
4. solve symbolically and numerically with units and a reasonableness check;
5. use vectors or calculus when the physical relationship requires them;
6. connect the result to a real situation and explain what changes when one input
   changes; and
7. complete the work independently before any answer comparison.

This implements the page 3 outcomes: inquiry and analysis, problem solving,
mathematical/computational methods, representations, modeling, real-world transfer,
and teamwork. The course lists zero laboratory hours, so no separate laboratory
stage is implied.

## Page-by-Page Disposition

| PDF page | Contents | Vault disposition |
|---:|---|---|
| 1 | course scope; instructor/contact/office hours; credits; zero lab hours; math prerequisite | academic scope routed above; course facts in [[source-map]]; math in [[math-readiness-path]] |
| 2 | no co/corequisite; Serway 10e; Day One Access; ISBN/publisher; Core IMPACTS area | textbook files registered in [[source-map]]; stale January access deadline is a verification gate |
| 3 | Core IMPACTS question/outcome/competencies; five course learning outcomes; homework heading | stage-wide outcome contract above |
| 4 | WebAssign homework, late work, collaboration; weekly quizzes; participation | course operations retained below; live D2L/WebAssign controls deadlines |
| 5 | exams, missed/make-up rules, grading scale | course operations retained; conflicting exam count/drop language is gated |
| 6 | assessment weights; dropped low HW/quiz/participation; Early Alerts; midterm grade; calendar heading | weights retained below; alerts/midterm are support/governance |
| 7 | calendar through Ch 5; attendance; missed in-class work; late work; missed exams | Ch 1-5 routed to Stages 1-5; dates and contradictions are gated |
| 8 | missed-exam exceptions; AI learning-aid and prohibited-submission policy | binding academic-integrity boundary in [[source-map]] and `CLAUDE.md` |
| 9 | modified operations and face-to-face continuity plan | check D2L/email during disruptions; governance only |
| 10 | virtual transition, adjusted deadlines, virtual office hours; integrity, conduct, accessibility | governance and academic-integrity boundary |
| 11 | accessibility contacts and reasonable accommodations | student-support governance; contact Student Disability Services early if needed |
| 12 | enrollment/attendance responsibility, copyright, FERPA | governance; no stage content |
| 13 | FERPA continuation, sexual misconduct/Title IX, withdrawal | governance; no stage content |
| 14 | withdrawal consequences; hardship and university-initiated withdrawals | governance; no stage content |
| 15 | military withdrawal; academic feedback; netiquette; delivery changes; academic resources | governance and support-resource register |
| 16 | academic/student resources; appeals/complaints; technology help desk | governance and support-resource register |
| 17 | technology make-up documentation and resources; wellness/career/health/veteran support | governance; save UITS ticket and notify instructor within 24 hours for a technical make-up request |
| 18 | instructor directory, office hours, degrees, teaching interests | instructor/context reference |
| 19 | teaching interest in conceptual foundations, quantitative reasoning, experimental skills, active learning, real-world applications, inclusion | stage-wide outcome contract and real-life anchors in [[learning-path]] |

## Stable Course Operations Recorded from the PDF

- Three credits: three class hours plus one recitation hour; zero laboratory hours.
- Prerequisite: C or better in MATH 1190, or MATH 1179 and MATH 1189.
- Homework uses WebAssign; group work is allowed, copying is not.
- Quizzes are described as weekly, 15 minutes, one attempt per question, with
  LockDown Browser and webcam; verify the live release/due schedule in D2L.
- Participation comes from active lecture/recitation engagement and group work.
- Published weights: homework 10%, quizzes 5%, participation 10%, three exams
  45%, cumulative final 30%. The lowest homework, quiz, and participation item is
  described as dropped.
- Published grade floors: A 90%, B 80%, C 70%, D 60%.
- AI is permitted only for studying, explanations, guided technique, examples, and
  clarification; no AI-generated content may be submitted for credit.
- Technical make-up requests require a UITS ticket number and instructor notice
  within 24 hours of the problem.

## Items That Still Require Live Verification

- Fall Day One Access opt-out deadline.
- Full calendar after September 16 / Chapter 5.
- The second Chapter 2 day/date entry and the August/September holiday labels.
- Whether late WebAssign work receives a 10% daily penalty or becomes unavailable
  once answers release.
- Exact unit-exam count, which exam score drops, and how the 45% exam category is
  calculated.
- Current office hours, assessment dates, and any changes made in D2L.

No missing syllabus physics topic remains unrouted. The unresolved items are course
operations or textbook section-depth decisions, not gaps in the semester concept
sequence.

## Cross-Section Verification, 2026-07-21

Two real Fall 2026 PHYS 2211 syllabi were obtained: Section 51 (Krishna Rana Magar)
and Section 55 (Swayamprabha Behera) — see `raw/syllabus/`. Neither is Chris's
registered Section 54 (flag #57, still open pending Aug 24 D2L confirmation), but
both are same-course, same-term, and give the first real look at pacing beyond
Chapter 5.

**What this confirms — no path change needed:**

- Topic order for Ch 1 through Ch 12 matches [[learning-path]]'s Stage 1-12
  sequence exactly in both sections. Ch 15 (SHM) also lands in the same relative
  position in both. This corroborates the existing stage order.
- `syllabus.pdf` (the original spine source, now archived — see the Source note
  above) was very likely an early, truncated capture of this same Section 55
  syllabus — identical instructor, identical grading weights, identical
  early-week dates. It is superseded by the new Section 55 file for anything
  past Chapter 5.

**What this surfaces as a real open question — tracked, not acted on:**

- Chris confirmed Ch 10-12 and Ch 14 in scope on 2026-06-25, based on the syllabus
  course-description text ("law of universal gravitation... falling objects and
  orbital motion," "fluid mechanics confirmed in the semester route"). Neither
  neighbor section's actual week-by-week calendar schedules Ch 14 (Fluid
  Mechanics) or Ch 38 (Relativity):
  - Section 55's calendar stops entirely after Ch 15 (SHM) with review weeks —
    never reaches Ch 13 (gravitation), Ch 14, Ch 16-17 (waves), or Ch 38.
  - Section 51's calendar does reach Ch 13 (week 5, paired with circular motion)
    and Ch 16-17 (weeks 12-14), but also never schedules Ch 14, and ends in
    review weeks without reaching Ch 38.
  - **Handling rule:** keep Stages 13-18 built and in the path — the course
    description in all three syllabi (original + both neighbors) names
    gravitation, waves, and relativity as course learning outcomes, and two
    sections omitting a chapter from a tentative calendar is not the same as it
    being out of scope. Re-verify against Section 54's real calendar the moment
    it exists in D2L (target: after Aug 24). Do not shrink the path on neighbor
    evidence alone.
- Grading structure differs materially between the two neighbors (Section 51:
  four unit exams, best 3 of 4, final 25%, attendance 7.5% + recitation 7.5%
  separately graded; Section 55: three unit exams, lowest dropped, final 30%,
  participation 10% covers both attendance and recitation). This mirrors the
  exam-count conflict already flagged in `syllabus.pdf` itself. Course
  operations only — no stage-path impact — but confirms the exam-count question
  cannot be resolved without Section 54's actual syllabus.
- **Soft pacing anchors from Section 55** (real 2026 dates, same instructor as
  the original spine, not binding for Section 54): Exam 1 ~Sep 25 (through Ch
  5/6), Exam 2 ~Oct 30 (through ~Ch 10), Exam 3 ~Nov 20 (through ~Ch 12/15),
  Thanksgiving break Nov 23-27, final review the last week. Useful for pacing
  Stage 5 → Stage 15 at a controlled rate; do not calendar these as Chris's
  actual exam dates.
