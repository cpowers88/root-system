...
Prior reading of a raw textbook chapter on the same general topic is background
exposure, not contamination of this measurement. What would contaminate it is
reading the wiki's own stage/drill file — the exact problems this loop tests —
immediately before the attempt. Reading is assigned only when Chris requests it
through `TUTOR.md`; no standing evening-reading workflow is active.
