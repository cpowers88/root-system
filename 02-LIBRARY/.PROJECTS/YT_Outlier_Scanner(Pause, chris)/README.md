...
- `market-report`, `market-worksheet`, `market-export`, `topic-report`, `report`, and `selftest` are local-only. They
...
- `market-worksheet` uses the same deduplicated ranking and writes two linked
  Markdown tables: scanner evidence plus blank human-only classification fields.
  It assigns no relevance, niche, fit, or content labels.
- `market-export` uses that same ranking to write a typed CSV for a private
  Looker Studio/Sheets rep plus a field dictionary. Blank numeric values remain
  blank for BI typing, untrusted title prefixes are neutralized against spreadsheet
  formulas, and search categories/topics remain provenance rather than human labels.
...
- [TOP_100_CLASSIFICATION_WORKSHEET.md](TOP_100_CLASSIFICATION_WORKSHEET.md) —
  canonical views-ranked evidence table plus 100 blank human-classification rows;
  generated by `market-worksheet`, not AI-labeled
- [LOOKER_STUDIO_MARKET_DATA.csv](LOOKER_STUDIO_MARKET_DATA.csv) — refreshable,
  views-ranked BI input generated offline by `market-export`
- [LOOKER_STUDIO_FIELD_DICTIONARY.md](LOOKER_STUDIO_FIELD_DICTIONARY.md) — field
  types, evidence boundaries, and the bounded first-dashboard rep
