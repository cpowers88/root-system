...
DB_PATH = Path(__file__).resolve().with_name("academic.db")
...
def seed_courses(connection):
    """Enter and reconcile the six registered Fall 2026 course components."""
    cursor = connection.cursor()

    courses = [
        ("PHYS2211", "Principles of Physics I", "Farhan Islam", 3),
        ("CSE1321", "Programming Problem Solving I", "Eun Sik Kim", 3),
        ("CSE1321L", "Programming Problem Solving I Lab", "Muhammad Usman", 1),
        ("ECON1000", "Contemporary Economic Issues", "Zeynep Kelani", 2),
        ("TCOM2010", "Technical Writing", "Lisa Diamond", 3),
        ("ENGR1000", "Introduction to Engineering", "Kamyar Raoufi", 1),
    ]

    cursor.executemany(
        """
        INSERT INTO courses (code, name, professor, credit_hours)
        VALUES (?, ?, ?, ?)
...
def show_week(connection):
    ...
    cursor.execute(
        ...
        WHERE assignments.due_date BETWEEN ? AND ?
          AND assignments.status NOT IN ('submitted', 'graded', 'archived')
    ...
    if assignments:
        for course, name, due_date, status, notes_file in assignments:
            print(
                f"- {due_date} | {course} | {name} | {status} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
    ...
    if readings:
        for course, chapter, pages, due_date, completed, notes_file in readings:
            ...
            print(
                f"- {due_date} | {course} | {chapter} | pages {pages or 'n/a'} | {status} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
    ...
    cursor.execute(
        ...
        WHERE tests.test_date BETWEEN ? AND ?
          AND COALESCE(tests.study_status, '') != 'archived'
    ...
    if tests:
        for course, name, test_date, chapters, study_status, notes_file in tests:
            print(
                f"- {test_date} | {course} | {name} | {chapters or 'chapters TBD'} | {study_status or 'not started'} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
...
def show_today(connection):
    ...
    if readings:
        for course, chapter, pages, completed, notes_file in readings:
            status = "complete" if completed else "not complete"
            print(
                f"- {course} | {chapter} | pages {pages or 'n/a'} | {status} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
    ...
    if tests:
        for course, name, chapters, study_status, notes_file in tests:
            print(
                f"- {course} | {name} | {chapters or 'chapters TBD'} | {study_status or 'not started'} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
...
def show_course(connection, course_code):
    ...
    if course is None:
        ...
        print("Try one of: PHYS2211, CSE1321, CSE1321L, ECON1000, TCOM2010, ENGR1000\n")
        return
    ...
    if assignments:
        for name, due_date, status, grade, notes_file in assignments:
            ...
            print(
                f"- {due_date} | {name} | {status} | {grade_display} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
    ...
    if readings:
        for chapter, pages, due_date, completed, notes_file in readings:
            status = "complete" if completed else "not complete"
            print(
                f"- {due_date} | {chapter} | pages {pages or 'n/a'} | {status} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
    ...
    if tests:
        for name, test_date, chapters, study_status, notes_file in tests:
            print(
                f"- {test_date} | {name} | {chapters or 'chapters TBD'} | {study_status or 'not started'} | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
...
def show_overdue(connection):
    ...
    if readings:
        for course, chapter, pages, due_date, completed, notes_file in readings:
            print(
                f"- {due_date} | {course} | {chapter} | pages {pages or 'n/a'} | not complete | {notes_file or 'no notes file'}"
            )
    else:
        print("- None")
...
def add_test(connection):
    ...
    study_status = input(
        "Study status, example not started / in progress / ready: "
    ).strip()
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """,
        (course_id, name, test_date, chapters, study_status, notes_file),
    )
...
def add_assignment(connection):
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """,
        (course_id, name, due_date, status, None, notes_file),
    )
...
def add_reading(connection):
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """,
        (course_id, chapter, pages, due_date, completed, notes_file),
    )
...
def main():
    ...
    parser.add_argument(
        "--week", action="store_true", help="Show everything due in the next 7 days"
    )
    ...
    parser.add_argument(
        "--today", action="store_true", help="Show everything due today"
    )
    parser.add_argument(
        "--overdue", action="store_true", help="Show overdue unfinished work"
    )
    parser.add_argument(
        "--course", type=str, help="Show everything for one course code"
    )
    parser.add_argument(
        "--add-test", action="store_true", help="Add a test interactively"
    )
    parser.add_argument(
        "--add-assignment", action="store_true", help="Add an assignment interactively"
    )
    parser.add_argument(
        "--add-reading", action="store_true", help="Add a reading interactively"
    )
