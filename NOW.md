...
1. **Python Stage 3 (Loops) first rep is MID-DRILL — resume here.** Session paused
   ...
   `S2P3.py` fix verified).
2. **Physics Stage 4 (Motion in Two Dimensions) is active** — start with
...
3. **Pre-semester rhythm (until syllabi land ~July 25, classes Aug 24):** one
   school proof block per day, then go **heavy on tech stacks** with the rest —
   SQL against the scanner's live SQLite DB, Looker Studio dashboard reps, the
   weekly landscape rep, and scanner build features. School-first ordering
   tightens back up when real deadlines exist, not before. (Chris-directed
   rebalance, 2026-07-16.)
...
| Technology | Make.com landscape rep and private Data Studio dashboard #1 are complete; scanner exercised Python/SQLite/API; pre-semester window is the heavy tech-stack window | Daily SQL on the scanner DB until tracker data arrives ~July 25; dashboard iteration #2 is optional |
...
| Date | Trigger |
|---|---|
| July 19 (Sun) | Metadata-regression recurrence review: rerun wiki close gate + `root_health.py`, require `comparison.new: []`, spot-check one new page per touched wiki (see DAILY 2026-07-16) |
| July 21 | OPP-20260714-01 (B2 conversation) review date |
| July 23 | OPP-20260716-01 (flip-margin replay) review date |
